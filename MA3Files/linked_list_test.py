...
class Test(unittest.TestCase):
    ...
    def test_remove(self): #
        ...
        self.assertEqual(lst.remove(6), True) #Remove last 
        self.assertEqual(lst.remove(7), False) #Try to remove non-existing node after the last node
        self.assertEqual(lst.remove(0), False) #Try to remove non-existing node before the first node
        self.assertEqual(lst.remove(1), True) #Remove first node
        self.assertEqual(lst.remove(1), True) #Remove secound node
        self.assertEqual(lst.remove(3), True) #Remove node in the middle
        self.assertEqual(lst.remove(3), False) #Try to remove non-existing node
        self.assertEqual(lst.remove(2), True)
        self.assertEqual(lst.remove(2), False)
        self.assertEqual(lst.remove(6), False) #Empty list

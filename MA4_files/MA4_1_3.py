...
def sphere_volume(n, d): # n is the number of points to generate, d is the number of dimensions of the sphere
...
def sphere_volume_parallel1(n,d,np):
    ...
    with future.ProcessPoolExecutor() as ex:
       result = ex.map(sphere_volume, [n]*np, [d]*np)
...
def sphere_volume_parallel2(n,d,np):
    ...
    with future.ProcessPoolExecutor() as ex:
        result = ex.map(sphere_volume, [n_per_process]*np, [d]*np)
    return sum(result) / np
...
def main():
    ...
    sphere_volume(n_seq, d)
    time_stop_seq = pc()
    print(f'Time of execution for splitting data: {time_stop_seq - time_start_seq} seconds')
